...
db = pymysql.connect(host='localhost', port=3306, user='root', passwd='disappear1!', db='gmarket', charset='utf8')
cursor = db.cursor()
...
def get_category(category_link, category_name):
    ...
    for sub_category in sub_categories:
        print(category_name, sub_category.get_text(),'http://corners.gmarket.co.kr' + sub_category['href'])\
...
categories = soup.select('.gbest-cate ul.by-group li a')
for category in categories:
    # 클릭했을때의 url tag 가 필요함.(href값) + 카테고리별 text
